...
const BookForm: React.FC<BookFormProps> = ({ book, isEditing }) => {
  ...
  const [formData, setFormData] = useState<{
    ...
    author: book?.author ?? '',
    genres:
      book?.bookGenres?.map(bg => bg.genreName ?? '').filter(Boolean) ?? [],
  ...
  const handleBookSuggestionSelect = (
    suggestion: (typeof bookSuggestions)[0]
  ): void => {
    setFormData(prev => ({
      ...prev,
  ...
  const handleInputChange = (
    ...
    // Special handling for publishedDate validation
    if (field === 'publishedDate' && typeof value === 'string') {
      ...
      if (!value) {
        ...
        if (!iso8601Date) {
          ...
          delete newErrors.publishedDate;
        }
      }
  ...
  const removeGenre = (genreToRemove: string): void => {
    handleInputChange(
      'genres',
      formData.genres.filter(g => g !== genreToRemove)
    );
  };

  const handleGenreKeyPress = (
  ...
  const handleSubmit = async (e: React.FormEvent): Promise<void> => {
    ...
    // Convert HTML date to ISO 8601 format before submission
    const iso8601Date = convertHtmlDateToIso8601(formData.publishedDate);
    if (!iso8601Date) {
                      ...
                      <div className="suggestion-content">
                        ...
                        {suggestion.publishedDate && (
                          <div className="suggestion-date">
                            Published:{' '}
                            {new Date(suggestion.publishedDate).getFullYear()}
                          </div>
                        )}
                        {suggestion.subjects &&
